...
clf = joblib.load(image_root + '/svm.pkl')


def return_maxkoflist(probs, k):
    top_k = probs[0].argsort()[-k:][::-1]
    probs_k = []
    lables_k = []
    for i, p in enumerate(top_k):
        probs_k.append(100 * probs[0][p])
...
def show_acc_preclass():
    ...
    for i, image_path in enumerate(images):
        ...
        probs, lables = disp_style_preds(feat[0])
    ...
    f1 = codecs.open(image_root + '/caffenet_svm/preimage_error_probs.txt', 'a+', 'utf-8')
    f1.write(str(preimage_error_probs))
    f1.close()

    f2 = codecs.open(image_root + '/caffenet_svm/preimage_error_labels.txt', 'a+', 'utf-8')
    f2.write(str(preimage_error_labels))
    f2.close()

    f3 = codecs.open(image_root + '/caffenet_svm/preimage_error_name.txt', 'a+', 'utf-8')
    ...
    plt.xlim([0, ind.size])
    plt.tight_layout()
    plt.savefig(image_root + '/caffenet_svm/classes_test_accuracy.png')
    plt.show()
...
def show_predict():
    ...
    for image_path in images:
        ...
        net.forward(start='conv1')
        feat = net.blobs['fc7'].data.copy()
        probas, labels = disp_style_preds(feat[0])
